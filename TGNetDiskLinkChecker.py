...
if __name__ == "__main__":
    ...
    link_checker_config = {
        # Telethon客户端配置
        "API_ID": config.get('api_id', 6627460),
        "API_HASH": config.get('api_hash', '27a53a0965e486a2bc1b1fcde473b1c4'),
        ...
        "TARGET_CHANNEL": config.get('forward_to_channel', ''),
        "PROXY": config.get('proxy', None),
        "BATCH_SIZE": 500,
        # 运行配置
        "DELETE_MODE": config.get('link_checker', {}).get('delete_mode', 2),
        "LIMIT": config.get('link_checker', {}).get('limit', 1000),
        "CONCURRENCY": config.get('link_checker', {}).get('concurrency', 20),
        "RECHECK": config.get('link_checker', {}).get('recheck', True),
        "NET_DISK_DOMAINS": config.get('link_checker', {}).get('net_disk_domains', ['pan.quark.cn'])
    }
    
    # 创建管理器实例
    manager = TelegramLinkManager(link_checker_config)
